...
@onready var _guitar_holder := $guitar_holder/remote_transform_3d as RemoteTransform3D
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if self.is_on_floor() and event.is_action_pressed("jump"):
		...
		if joypad_motion_event.axis == 2:
			self.rotate_y(joypad_motion_event.axis_value * Game.joypad_look_sensitivity)
		elif joypad_motion_event.axis == 3:
			self._camera.rotate_x(joypad_motion_event.axis_value * Game.joypad_look_sensitivity)


func _input(event: InputEvent) -> void:
	if (
		not self._is_player_controlling
		and self._held_prop is GuitarProp
...
func _try_interact() -> void:
	...
	if picked_object is SceneletTrigger:
		...
		if picked_object is GuitarProp:
			self._is_player_controlling = false
			picked_object.start_playing()
			self._guitar_holder.remote_path = self._guitar_holder.get_path_to(picked_object)
			self.create_tween().tween_property(
				self._camera, "rotation_degrees", Vector3(-32.7, 0.0, 0.0), 1.0
			)
...
func _drop() -> void:
	if self._held_prop is GuitarProp:
		self._is_player_controlling = true
		self._held_prop.stop_playing()
		self._guitar_holder.remote_path = ^""
...
func _pick() -> void:
	...
	self._can_pickup = picked_object is PhysicsProp and not self._held_prop
